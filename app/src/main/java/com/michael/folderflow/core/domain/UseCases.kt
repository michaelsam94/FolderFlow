...
class ScanInstalledAppsUseCase(private val appRepository: AppRepository) {
    suspend operator fun invoke(mockIfLowCount: Boolean = true) {
        appRepository.scanAndSyncApps(mockIfLowCount)
    }
}
...
class UpdateFolderUseCase(private val folderRepository: FolderRepository) {
    suspend operator fun invoke(
        folderId: String,
        name: String,
        iconEmoji: String,
        ...
        folderRepository.updateFolder(folderId, name, iconEmoji, colorToken, rules)
    }
}
...
class RemoveAppUseCase(private val appRepository: AppRepository) {
